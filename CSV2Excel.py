...
def run_enrichment_analysis(destination_folder, formulations_sheet, csv_filepath, sorted_cells,\
							x_percent, cell_type, percentile=99.9):
	'''
	run_enrichment_analysis : driver function, uses all other functions to create enrichment analysis
		inputs:
				destination_folder : user specified path to the folder where the user wants the excel file
									created to be saved
				formulations_sheet : user specified file path to excel spreadsheet with formulation sheet
									only
	...
	d_df_components_top, d_df_components_bottom = top_bottom_enrichment(destination_file,\
														cell_type, df_averaged, x_percent)
	...
	create_net_enrichment_factor(destination_file, d_df_components_averaged,\
								d_df_components_top, d_df_components_bottom, cell_type)
...
def create_net_enrichment_factor(destination_file, d_df_components_averaged,\
	...
	create_net_enrichment_factor: creates excel sheet with all enrichment analysis (averaged, top,\
								bottom, raw enrichment and net enrichment factor) named "Net Enrichment Factors"
		inputs:
				destination_file : directory of the excel spreadsheet
				d_df_components_averaged : dictionary with all dataframes of all enrichment calculations of\
											df_averaged
				d_df_components_top : dictionary with all dataframes of all enrichment calculations of df_top
				d_df_components_bottom : dictionary with all dataframes of all enrichment calculations of\
											df_bottom
				cell_type : user specified cell type to sort by
	'''
	...
	d_df_component_net_enrichment, d_raw_enrichment_top, d_raw_enrichment_bottom = \
				net_enrichment_factor(d_df_components_averaged, d_df_components_top,\
									d_df_components_bottom, cell_type)
	dict_df_raw_enrichment_top = dict_list_to_dict_df(d_raw_enrichment_top, cell_type)
	dict_df_raw_enrichment_bottom = dict_list_to_dict_df(d_raw_enrichment_bottom, cell_type)
...
def net_enrichment_factor(d_df_components_averaged, d_df_components_top,\
	...
	net_enrichment_factor: creates dataframes for best and worst performing LNPs, counts and their
		...
		output:
				...
				d_raw_enrichment_factors_top
		 : dictionary with dataframes of raw enrichment of top
												performing LNPs
				d_raw_enrichment_factors_bottom
		 : dictionary with dataframes of raw enrichment of
	...
	for component in dict_component_net_enrichment_factor:
		...
		d_df_component_net_enrichment[component] = pd.DataFrame(data=np_temporary,\
																columns=[component, cell_type])

	return d_df_component_net_enrichment, d_raw_enrichment_factors_top, d_raw_enrichment_factors_bottom
	...
	raw_enrichment_factor: creates dataframes for best and worst performing LNPs, counts and their
		...
		inputs:
				...
				d_df_components_top_bottom : dictionary with all dataframes of all enrichment
											calculations of df_top_bottom_cell_type
		output:
...
def top_bottom_enrichment(destination_file, cell_type, df_averaged, x_percent):
	...
	d_df_components_top = create_enrichment_tables(destination_file, df_averaged, df_top, cell_type,\
													"Top")
...
def create_enrichment_tables(destination_file, df_averaged, df_top_bottom_cell_type=None,\
							cell_type=None, top_or_bottom=None):
	'''
	create_enrichment_tables: creates excel sheet with formulation enrichment tables of averaged
							...
							df_top_bottom_cell_type value is passed) named "Form Enrichment" (or
							"Form Enrichment" + cell_type + top_or_bottom if df_top_bottom_cell_type
							provided)
		inputs:
				destination_file : directory of the excel spreadsheet
				df_averaged : dataframe with averaged normalized counts by cell type
				df_top_bottom_cell_type : dataframe of either top or bottom performing LNPs by specified cell
				type (default = None)
				cell_type : user specified cell type to sort by (default = None)
				top_or_bottom : specifies if enrichment is for top or bottom performing LNPs by specified cell
				type(default = None)
		output:
				dict_df_components : dictionary with all data frames of all enrichment calculations of
				df_averaged (or df_top_bottom_cell_type if inputted)
	'''
	...
	with pd.ExcelWriter(destination_file, engine="openpyxl", mode="a") as writer:
		...
		for index in range(len(list_enrichments)//2):
			dict_df_components[list_enrichments[index]].to_excel(writer, sheet_name=enrichment_sheet,\
											startrow=current_row_1, startcol=off_set + 2, index=False)
			dict_df_components[list_enrichments[index + 4]].to_excel(writer, sheet_name=enrichment_sheet,\
											startrow=current_row_2, startcol=off_set + 6, index=False)
...
def sort_norm_counts(cell_type, df_averaged):
	'''
	sort_norm_counts: creates dataframe with normalized counts sorted in descending order by specified
					cell type
		inputs:
				cell_type : user specified cell type to sort by
				df_averaged : dataframe with averaged normalized counts by cell type
...
def get_all_enrichments(df_averaged, df_top_bottom_cell_type):
	...
	get_all_enrichments: calculated enrichment by component or component_ratio
		inputs:
				df_averaged : dataframe with averaged normalized counts by cell type
				df_top_bottom_cell_type : dataframe of either top or bottom performing LNPs by specified
				cell_type (optional input)
		output:
...
def average_normalized_counts(df_merged, organized_columns, sorted_cells, destination_file):
	'''
	average_normalized_counts : creates and returns a dataframe with averaged normalized counts by cell
	type and appends it to excel spreadsheet on a sheet named "Averaged Norm Counts"
		inputs:
...
def merge_formulations_and_norm_counts(df_formulations, df_norm_counts, organized_columns,\
										destination_file, add_to_excel=False):
	'''
...
def organize_by_cell_type(sample_columns, sorted_cells):
	'''
	organize_by_cell_type : gets data fram with normalized counts, creates a dataframe without outliers
	...
	for cell_type in sorted_cells:
		for column in sample_columns:
			# if current cell_type is in the name of the current column (e.g: if "SB" in "AD SB102")
			if cell_type in column:
				organized_columns.append(column)

	# number of repeats per cell type, should be same for all
	count_repeats = len(sample_columns)//len(sorted_cells)
...
def create_df_norm_no_outliers(df_norm_counts, percentile):
	...
	n_at_percentile = np.percentile(df_norm_no_outliers.to_numpy(), percentile)
	...
	for row in rows:
		for column in sample_columns:
			# if value at location [row,column] if greater (outlier)
			if df_norm_no_outliers.at[row, column] >= n_at_percentile:
...
def create_df_norm_counts(csv_filepath, destination_file):
	...
	it to destination_file on a sheet named "Normalized Counts"
		inputs:
